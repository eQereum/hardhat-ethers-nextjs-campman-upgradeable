...
describe('Campaign Contract -----------------------------------------------------', () => {
  ...
  before('deploying Campaigns', async () => {
    ...
    Campiagn = await ethers.getContractFactory('Campaign');

    [hardhatCampaign1, hardhatCampaign2, hardhatCampaign3] = await Promise.all([
      Campiagn.deploy(owner.getAddress(), factory.getAddress(), ethers.utils.formatBytes32String('camp1'), 'desc1', 'banner1', min, period1, manager1.getAddress()),
      Campiagn.deploy(owner.getAddress(), factory.getAddress(), ethers.utils.formatBytes32String('camp2'), 'desc2', 'banner2', min2, period2, manager2.getAddress()),
      Campiagn.deploy(owner.getAddress(), factory.getAddress(), ethers.utils.formatBytes32String('camp3'), 'desc3', 'banner3', min3, period3, manager3.getAddress()),
    ]);
  ...
  describe('contribute()', () => {
    ...
    it('should emit ContributionLog & confirm contribution value', async () => {
      const tx = await Promise.all([
        hardhatCampaign1.connect(investor1).contribute({ value: min }),
        hardhatCampaign1.connect(investor1).contribute({ value: min }),
        hardhatCampaign1.connect(investor2).contribute({ value: min2 }),
      ]);
      const receipt = await Promise.all([tx[0].wait(), tx[1].wait(), tx[2].wait()]);

      expect(receipt[0].events[1].args.value).to.be.equal(min);
      expect(receipt[0].events[1].args.investor).to.be.equal(await investor1.getAddress());
      expect(receipt[1].events[1].args.value).to.be.equal(min);
      expect(receipt[1].events[1].args.investor).to.be.equal(await investor1.getAddress());
      expect(receipt[2].events[1].args.value).to.be.equal(min2);
      expect(receipt[2].events[1].args.investor).to.be.equal(await investor2.getAddress());
    });
    ...
    it('should confirm isInvestor[investor1], isInvestor[investor2]', async () => {
      expect(await hardhatCampaign1.isInvestor(investor1.getAddress())).to.be.equal(true);
      expect(await hardhatCampaign1.isInvestor(investor2.getAddress())).to.be.equal(true);
    });

    it('should confirm contributionperinvestor[investor1]', async () => {
      expect(await hardhatCampaign1.contributionPerInvestor(investor1.getAddress())).to.be.equal(min2);
      expect(await hardhatCampaign1.contributionPerInvestor(investor2.getAddress())).to.be.equal(min2);
    });
  ...
  describe('rejectCamapign()', () => {
    ...
    it('should revert because caller is not owner or factory', async () => {
      await Promise.all[
        (expect(hardhatCampaign1.connect(investor1).rejectCampaign()).to.be.revertedWith('only owner can call this function'),
        expect(hardhatCampaign1.connect(manager1).rejectCampaign()).to.be.revertedWith('only owner can call this function'))
      ];
    });
    ...
    it('should emit createPoolLog event which pool address is not 0x0 & not campaign2 address because campaign2 has funds in it', async () => {
      ...
      expect(receipt.events[0].args.message).to.be.equal('Pool Created - Succeccfully Rejected');
      expect(receipt.events[0].args.rejectedAddress).to.be.equal(deployedAddress2);
      expect(receipt.events[0].args.poolAddress).to.not.equal(deployedAddress2);
  ...
  describe('createRequest()', () => {
    const description = [
      Array(257).fill(0).toString().replaceAll(',', ''),
      Array(10).fill(1).toString().replaceAll(',', ''),
      Array(20).fill(2).toString().replaceAll(',', ''),
      Array(30).fill(3).toString().replaceAll(',', ''),
    ];
    ...
    it('should revert because contribution is not over', async () => {
      await expect(hardhatCampaign1.connect(manager1).createRequest(description[1], value[1], recipient.getAddress())).to.be.revertedWith('you have to wait until the contributiom period is over !!!');
    });
    ...
    it('should revert when called by other accounts than manager', async () => {
      await expect(
        Promise.all([
          hardhatCampaign1.connect(owner).createRequest(description[1], value[1], recipient.getAddress()),
          hardhatCampaign1.connect(manager2).createRequest(description[1], value[1], recipient.getAddress()),
          hardhatCampaign1.connect(investor1).createRequest(description[1], value[1], recipient.getAddress()),
          hardhatCampaign1.connect(attacker1).createRequest(description[1], value[1], recipient.getAddress()),
          hardhatCampaign1.connect(recipient).createRequest(description[1], value[1], recipient.getAddress()),
        ])
    ...
    it('should revert when called the closed campaign', async () => {
      await Promise.all([
        expect(hardhatCampaign2.connect(manager2).createRequest(description[2], value[2], recipient.getAddress())).to.be.revertedWith('campaign is not available anymore'),
        expect(hardhatCampaign3.connect(manager3).createRequest(description[2], value[3], recipient.getAddress())).to.be.revertedWith('campaign is not available anymore'),
      ]);
    });

    it('should revert because description length is too big', async () => {
      await expect(hardhatCampaign1.connect(manager1).createRequest(description[0], value[1], recipient.getAddress())).to.be.revertedWith('request description is too big');
    });

    it('should revert because requested value is more than campaigns avalilable balance', async () => {
      await expect(hardhatCampaign1.connect(manager1).createRequest(description[1], value[0], recipient.getAddress())).to.be.revertedWith('requested value is more than available balance');
    });

    it('should create request succesfully', async () => {
      const tx = await Promise.all([
        hardhatCampaign1.connect(manager1).createRequest(description[1], value[1], recipient.getAddress()),
        hardhatCampaign1.connect(manager1).createRequest(description[2], value[2], recipient.getAddress()),
        hardhatCampaign1.connect(manager1).createRequest(description[3], value[3], recipient.getAddress()),
      ]);
      const receipt = await Promise.all([tx[0].wait(), tx[1].wait(), tx[2].wait()]);
      ...
      expect(receipt[0].events[0].args.recipient).to.be.equal(await recipient.getAddress());
      ...
      expect(receipt[1].events[0].args.recipient).to.be.equal(await recipient.getAddress());
      ...
      expect(receipt[2].events[0].args.recipient).to.be.equal(await recipient.getAddress());
      ...
      expect(request[0].recipient).to.be.equal(await recipient.getAddress());
      ...
      expect(request[1].recipient).to.be.equal(await recipient.getAddress());
      ...
      expect(request[2].recipient).to.be.equal(await recipient.getAddress());
  ...
  describe('approveRequest()', () => {
    ...
    it('should emit ApproveRequestLog()', async () => {
      ...
      expect(receipt[0].events[0].args.approver).to.be.equal(await investor1.getAddress());
      expect(receipt[1].events[0].args.index).to.be.equal(0);
      expect(receipt[1].events[0].args.approver).to.be.equal(await investor2.getAddress());
    });
  ...
  describe('finalizeRequest()', () => {
    ...
    it('should emit FinalizeRequestLog(index,recipient)', async () => {
      await expect(hardhatCampaign1.connect(manager1).finalizeRequest(0))
        ...
        .withArgs(0, await recipient.getAddress());
    });
  });
